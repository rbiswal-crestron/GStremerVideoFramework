...
#import "CSMobileVideoStreaming/CSMobileVideoStreaming-Swift.h"
...
-(void) run_app_pipeline_threaded:(NSString*)uri{
    [self run_app_pipeline:uri];
    return;
...
static void state_changed_cb (GstBus *bus, GstMessage *msg, GStreamerBackend *self)
{
    ...
    if (GST_MESSAGE_SRC (msg) == GST_OBJECT (self->pipeline)) {
        ...
        gchar *message = g_strdup_printf("%s\n%d", gst_element_state_get_name(new_state),new_state);
...
-(void) run_app_pipeline:(NSString*)uri
{
    GSource *bus_source;
    GError *error = NULL;
   ...
   // pipeline = gst_parse_launch("playbin \(char_uri", &error);
    
    NSString *url = [NSString stringWithFormat:@"playbin uri=%@", uri];
    
    const char *char_uri = [url UTF8String];
    
    pipeline = gst_parse_launch(char_uri, &error);
